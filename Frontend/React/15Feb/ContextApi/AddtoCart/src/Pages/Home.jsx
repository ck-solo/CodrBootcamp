...
function Home({ setCart, cart }) {
  ...
  let addtoCart = (id) => {
    ...
    setCart((prev) => {
      let isexist = prev.find((val) => val.id === product.id);
      console.log(isexist);
  ...
  return (
    <div className="bg-gray-200 flex flex-wrap gap-20">
      {products.map((elem) => {
        return <ProductCard key={elem.id} elem={elem} addtoCart={addtoCart} />;
      })}
    </div>
  );
}
