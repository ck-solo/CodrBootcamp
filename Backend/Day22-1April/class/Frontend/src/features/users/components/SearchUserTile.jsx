...
const SearchUserTile = ({ user }) => {
    ...
    return (
        <div key={user._id} className="flex items-center justify-between p-3 rounded-xl hover:bg-[#1A1A28] transition-colors cursor-pointer border border-transparent hover:border-white/5">
            <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full overflow-hidden bg-[#151520] border-[1.5px] border-white/10 shrink-0 shadow-sm shadow-purple-900/10">
                    <img
                        src={user.profilePicture || `https://ui-avatars.com/api/?name=${user.username || 'User'}&background=151520&color=9ca3af`}
                        alt={user.username}
